    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl p-8 border border-slate-200 text-center">
        <div className="w-24 h-24 mx-auto mb-4 rounded-full bg-gradient-to-br from-teal-500 to-teal-600 flex items-center justify-center">
          <span className="text-3xl font-bold text-white">{getInitials(profile.nickname)}</span>
        </div>
        <h1 className="text-2xl font-bold text-slate-900 mb-1 font-['Space_Grotesk']">{profile.nickname}</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-6 border border-slate-200 text-center">
          ...
          <p className="text-2xl font-bold text-slate-900 font-['Azeret_Mono']">{profile.stats?.total_quizzes || 0}</p>
          <p className="text-xs text-slate-600 mt-1">Quizzes Played</p>
        </div>
        <div className="bg-white rounded-xl p-6 border border-slate-200 text-center">
          <Target className="w-8 h-8 text-teal-600 mx-auto mb-2" />
          <p className="text-2xl font-bold text-slate-900 font-['Azeret_Mono']">{profile.stats?.unique_quizzes || 0}</p>
          <p className="text-xs text-slate-600 mt-1">Unique Quizzes</p>
        </div>
        <div className="bg-white rounded-xl p-6 border border-slate-200 text-center">
